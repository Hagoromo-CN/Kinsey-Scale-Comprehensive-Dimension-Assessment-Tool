...
5. 量表结果可能随个人经历、认知变化而改变，并非一成不变。
        ...
        "next_btn": "➡️ 下一题",
        "submit_btn": "📤 提交所有答案",
        # 【关键修改1】中文界面：按钮显示 "Switch to the English"
...
# ===================== 50题双语题库（核心35+社会5+无性恋10） =====================
QUESTIONS_50 = [
    # -------------- 核心性取向维度（Core Sexual Orientation）：1-35题 --------------
    (
...
TOTAL_QUESTIONS = 50  # 总题数50
CORE_END = 35         # 核心性取向截止35题（索引0-34）
STRESS_END = 40       # 社会压力截止40题（索引35-39）
ASEX_START = 40       # 无性恋从40题开始（索引40-49）

# ===================== 核心计分函数（适配50题，双语结果） =====================
def calculate_results(answers, lang):
    # 拆分维度：核心35(0-34) + 社会5(35-39) + 无性恋10(40-49)
    ...
    core_grade = round(core_avg) if core_avg <=6 else 6  # 限制等级0-6
...
# ===================== 核心交互逻辑（含中英文切换，进度/答案不丢失） =====================
def init_quiz(lang):
    """初始化答题：加载第1题，初始化状态，适配当前语言"""
    q_text_cn, q_text_en = QUESTIONS_50[0][0]
    q_opt_cn, q_opt_en = QUESTIONS_50[0][1]
    q_text = q_text_cn if lang == "cn" else q_text_en
    q_opt = q_opt_cn if lang == "cn" else q_opt_en
    ui = UI_TEXT[lang]
    return (
        gr.update(value=q_text),
        gr.update(choices=q_opt, value=None, label=ui["select_ans"]),
...
def switch_language(current_lang, current_idx, answers_list):
    """切换语言：更新所有文字，保留进度/已选答案"""
    new_lang = "en" if current_lang == "cn" else "cn"
    ui = UI_TEXT[new_lang]
    # 加载当前题的双语内容
    q_text_cn, q_text_en = QUESTIONS_50[current_idx][0]
    q_opt_cn, q_opt_en = QUESTIONS_50[current_idx][1]
    q_text = q_text_cn if new_lang == "cn" else q_text_en
    q_opt = q_opt_cn if new_lang == "cn" else q_opt_en
    # 按钮显示控制
...
def prev_question(current_idx, answers, current_choice, lang):
    """上一题：保存答案，加载上一题，适配当前语言"""
    # 空答案不保存（避免覆盖已选答案）
    if current_choice is not None:
        answers[current_idx] = current_choice
    new_idx = current_idx - 1
    # 加载当前语言的题目/选项
    q_text_cn, q_text_en = QUESTIONS_50[new_idx][0]
    q_opt_cn, q_opt_en = QUESTIONS_50[new_idx][1]
    ui = UI_TEXT[lang]
    q_text = q_text_cn if lang == "cn" else q_text_en
    q_opt = q_opt_cn if lang == "cn" else q_opt_en
    # 按钮控制
    ...
    return (
        gr.update(value=q_text),
        gr.update(choices=q_opt, label=ui["select_ans"], value=answers[new_idx]),
        ...
        new_idx,
        answers.copy()  # 6.x要求返回新列表（不可变）
    )

def next_question(current_idx, answers, current_choice, lang):
    """下一题：保存答案，加载下一题，适配当前语言"""
    # 空答案不保存（避免覆盖已选答案）
    if current_choice is not None:
        answers[current_idx] = current_choice
    new_idx = current_idx + 1
...
def submit_quiz(answers, current_choice, current_idx, lang):
    """提交答案：验证+计分，适配当前语言"""
    # 保存最后一题答案
    if current_choice is not None:
        answers[current_idx] = current_choice
    # 检查是否有未答题
    if None in answers:
        empty_idx = answers.index(None) + 1
        return UI_TEXT[lang]["empty_tip"].format(empty_idx)
    # 转换答案为分数（根据当前语言匹配选项索引）
    answer_scores = []
    ...
    # 计算双语结果
    return calculate_results(answer_scores, lang)

# ===================== Gradio前端构建（适配Gradio 6.x） =====================
def create_gradio_interface():
    with gr.Blocks(title=UI_TEXT["cn"]["title"]) as demo:
        # 语言状态：默认中文（cn/en）
        ...
        # 顶部按钮：语言切换 + 开始评估（6.x调整variant属性）
        with gr.Row():
            # 【初始化按钮文本】默认中文界面显示 Switch to the English
        ...
        # 答题/结果区域（6.x中Tab的label初始化后不建议动态修改，简化为固定标签）
        ...
        # ===================== 交互事件绑定（适配6.x，移除post_fn，改用then()） =====================
        # 语言切换
        lang_switch_btn.click(
            fn=switch_language,
            inputs=[current_lang, current_idx, answers_list],
            outputs=[
        ...
        # 开始评估：先初始化答题，再显示答题Tab（用then()替代post_fn）
        start_btn.click(
            fn=init_quiz,
            inputs=[current_lang],
            outputs=[
                question_text, current_choice, progress_text,
                prev_btn, next_btn, submit_btn, current_idx, answers_list
            ]
        ).then(
            fn=show_quiz_tab,
            inputs=[],
        ...
        # 提交答案：先计算结果，再显示结果Tab（用then()替代post_fn）
        submit_btn.click(
            fn=submit_quiz,
...
if __name__ == "__main__":
    ...
    # 启动参数：适配6.x（移除quiet，调整参数）
    demo.launch(
        server_name="127.0.0.1",
        server_port=7860,
        share=False,
        inbrowser=True,
        show_error=False,  # 替代6.x中废弃的quiet=True
        #show_api=False     # 隐藏API文档（6.x新增）
    )
    # 控制台提示（双语适配，默认中文）
    ui_cn = UI_TEXT["cn"]
    print("="*80)
